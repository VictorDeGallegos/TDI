...
<table border=1>
    <tr><th>Remitente</th><th>Mensaje</th><th>Borrar</th><th>Fecha</th></tr>
<%boolean men = false;
...
if( mensajes != null )
  ...
  for( int i=0; i<mensajes.size(); i++ ) {
    ...
    if (( m.getDestino()).equalsIgnoreCase( nombre )) {
      men = true;%>
<tr><td><%= m.getRemite()%></td><td><%= m.getTexto()%></td><td><a href="borrar.jsp?nombre=<%=nombre%>&remite=<%=m.getRemite()%>&texto=<%=m.getTexto()%>"> <img style="width: 25px; height: 25px;" src="images/Borrar_mail.png"></a></td><td><%= m.getFecha()%></td></tr>
    <%}
  }
if ( !men ) {%>
